...
nsim <- 1000
nsamp <- 1000
simchi <- simchiRand <- numeric(nsim)
chiBins <- chiBinRand <- vector(mode = "list", length = nsim)
criteria <- makeCriteria(expn <= 10, n <= 10, depth >= 6)
for (ii in 1:nsim) {
    tempbinchi <- binner(sample(1:nsamp), sample(1:nsamp),
                         stopper = function(bns) stopper(bns, criteria),
                         splitter = function(bn) {
                             maxScoreSplit(bn, chiScores)
                         })
    tempbinr <- binner(sample(1:nsamp), sample(1:nsamp),
                       stopper = function(bns) stopper(bns, criteria),
                       splitter = function(bn) {
                           maxScoreSplit(bn, randScores)
                       })
    simchi[ii] <- binChi(tempbinchi)$stat
    chiBins[[ii]] <- tempbinchi
    simchiRand[ii] <- binChi(tempbinr)$stat
    chiBinRand[[ii]] <- tempbinr
...
narrowPlot(xgrid = seq(0, 3, by = 0.5),
           xlab = expression(log[10]~"(Number of bins)"),
           ygrid = seq(0, 3.5, by = 0.5), # xlim = c(0, 550),
           ylab = expression(log[10]~{"("~chi^2~statistic~")"}))
points(log(depthSeq.rnd["nbin",,],10), log(depthSeq.rnd["chi",,],10),
       col = adjustcolor(hcl.colors(9, "Dark 2"), 0.3), pch = 20)
for (ii in 1:9) points(mean(depthSeq.rnd["nbin",ii,]),
                       mean(depthSeq.rnd["chi",ii,]), col = "black",
...
narrowPlot(xgrid = seq(0, 3, by = 0.6),
           xlab = expression(log[10]~"(Number of bins)"),
           ygrid = seq(-4, -1, by = 1),
           ylab = expression(log[10]~"(Mutual information score)"))
points(log(depthSeq.chi["nbin",,], 10),
       log(depthSeq.chi["mi",,], 10), pch = 20,
       col = adjustcolor(hcl.colors(9, "Dark 2"), 0.3))
for (ii in 1:9) points(log(mean(depthSeq.chi["nbin",ii,]), 10),
...
xx[,i] <- x
yy[,i] <- y
  ### I could rescale y to put it in the -1,1 box??, or not
...
#library("energy")
#stats <- matrix(NA, 6, 4)
#dimnames(stats)[[2]] <- c( "Pearson", "Spearman", "Distance", "Distance-P" )
#for( i in 1:6 )
# {
#  aa <- uu[,i]
#  bb <- vv[,i]
#  stats[i,1] <- cor( aa, bb )
#  stats[i,2] <- cor( rank(aa), rank(bb) )
#
#  stats[i,3] <- dcor( aa, bb )
#  stats[i,4] <- dcov.test( aa, bb, R=10000)$p.value
#  print(i)
# }
#
tmp <- format( round( c( stats[,1], stats[,3]), 3 ), digits=2 )
pc <- tmp[1:6]
dc <- tmp[7:12]

m <- 1.4
pdf( file="dc-3.pdf", height=m, width=6*m )
par( mfrow=c(1,6), mar=c(8,1,1,1)/2 )  ## drop the 2nd one
for( i in 1:6 )
 {
  plot( uu[,i], vv[,i], xlab="", ylab="", axes=F  )
  mtext( side=1, text=pc[i], line=1 )
  mtext( side=1, text=dc[i], line=3 )
 }
dev.off()
## TO DO
