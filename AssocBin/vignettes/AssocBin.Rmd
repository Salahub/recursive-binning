...
This data set records more than a dozen measurements of heart function and demographics collected from patients admitted to several US hospitals. Of particular interest is the `num` variable, which records the presence and severity of heart disease in each patient. Patients with a `num` value of zero have healthy hearts and larger values of `num` indicate increasingly severe heart disease. We first clean the data somewhat by removing mostly missing variables and omitting records that are mostly incomplete.
...
Individual pairwise relationships can now be investigated using `depDisplay`, which displays a version of the scatterplot between a pair of variables processed to highlight regions with more or fewer points than we would expect if the pair was independent. Here, we'll use it to look at the relationship of a few variables with `num`, starting with `sex`: the sex of the patient.

```{r, fig.width=7, fig.height=5}
depDisplay(heartClean$sex, heartClean$num)
```

Optional arguments can be supplied to make the plot easier to read and understand in the same way as for many other R objects:

```{r, fig.width=7, fig.height=5}
depDisplay(heartClean$sex, heartClean$num, xlab = "Sex", ylab = "Heart disease severity", pch = 20)
```

Labels and point types aside, reading this plot requires some understanding of the underlying logic of the algorithm. In the case of `sex` and `num`, both categorical variables, the plot generated is analogous to a mosaic plot. The marginal proportions of each categorical level are reflected by the size of the corresponding labelled segment on that margin. Hence, the area of each cell is proportional to the number of points which would occur with the corresponding combination of categorical levels under independence. In effect, the cells are a visualization of the joint distribution of these variables under independence.

To overlay the observed counts on this imagined joint distribution, saturation is employed. To determine the level of saturation for the cell with level $i$ on the first categorical variable and level $j$ on the second, the Pearson residual
$$r_{ij} = \frac{o_{ij} - e_{ij}}{\sqrt{e_{ij}}}$$
Each cell's observed count is compared to the expected count and colouring is added if the difference between the two is larger  Saturation is then added to indicate whether more or fewer points occur in that cell than expected: red indicates an excess of points while blue indicates a lack. Cells which remain white have a small enough difference 
