...
for (ii in 1:6) {
  nm <- names(assocs$binning)[[pord[ii]]]
  pr <- strsplit(nm, "\\:")[[1]]
  plotBinning(assocs$binning[[pord[ii]]],
              fill = residualFill(assocs$binning[[pord[ii]]],
                                  maxRes = maxRes),
              xlab = pr[1], ylab = pr[2], pch = ".")
}
```

One of the 

We can also customize this behaviour by changing the splitting function called by the function.

```{r}
maxCatCon <- function(x, y, sFn) {
  uniBinner(x, y, stopper = sFn, 
            splitter = function(bn) uniMaxScoreSplit(bn, chiScores))
}
maxConCon <- function(x, y, sFn) {
  binner(x, y, stopper = sFn,
         splitter = function(bn) maxScoreSplit(bn, chiScores))
}
maxPairs <- pairwiseAssociation(data = heartClean, stopCriteria = stopCrits,
                                dualContinuous = maxConCon,
                                categoricalContinuous = maxCatCon,
                                returnBinnings = TRUE)
```

```{r}
pvalsMax <- pchisq(sapply(maxPairs$statistic, function(x) x$stat),
                df = maxPairs$df, lower.tail = FALSE)
pordMax <- order(pvals, decreasing = FALSE)
maxRes <- max(sapply(maxPairs$statistic, function(x) max(x$residuals)))
for (ii in 1:6) {
  nm <- names(maxPairs$binning)[[pordMax[ii]]]
  pr <- strsplit(nm, "\\:")[[1]]
  plotBinning(maxPairs$binning[[pordMax[ii]]],
              fill = residualFill(maxPairs$binning[[pordMax[ii]]],
                                  maxRes = maxRes),
              xlab = pr[1], ylab = pr[2], pch = ".")
}
```