## adapted from code by Marius Hofert

## This script loads and process constituent data of the S&P 500
...
sector_panel_function <- function(x, sectors, subsectors,
    ...
    function(...){
        ...
        ## Sectors
        for(s in sectors) {
            panel.lines(x = c(s, s), y = c(s, nrow(x)), col = colsec,
                        lwd=lwdsec, lty = 2)
            panel.lines(x = c(1, s), y = c(s, s), col = colsec,
                        lwd=lwdsec, lty = 2)
        }
        ## Subsectors
        for(s in subsectors) {
            panel.lines(x = c(s, s), y = c(s, nrow(x)),
                        col = colsubs, lwd = lwdsubs)
            panel.lines(x = c(1, s), y = c(s, s), col = colsubs,
                        lwd = lwdsubs)
        }
    }
}

##' @title Compute Pairwise Rosenblatt Transformed Data Based on a
##' Fitted Full or Bivariate t Copula and Compute Pairwise p-values
##' via an AD Test after Mapping to a K_2
##' @param U pseudo-observations
##' @param P correlation matrix of the fitted (full or pairwise) t
##' copula(s)
##' @param nu matrix of degrees of freedom of the fitted (full or
##' pairwise) t copula(s)
##' @return list with
##'         1) an (i,j,n,2)-array containing the row index i,
##'            col index j, U[,i] and C(U[,j] | U[,i])
##'         2) an (i,j,.)-array containing the corresponding p-values
##'            (of the AD tests after mapping the Rosenblatt
##'             transformed data to a K_2 distribution)
##'         3) a (d,d)-matrix containing the p-values
##' @author Marius Hofert
##' Note: We should actually work with gofCopula(, method = "Sn"),
##' but see the the note in Section 7.2 below. Also, gofCopula() is
...
## Read the S&P 500 constituent data (composition of the S&P 500 as
## of 2016-01-03)
...
## Keep only the time series without missing data
keep <- apply(x, 2, function(x.) mean(is.na(x.)) <= 0) # keep those
                                                       # without NAs
...
## We first fit ARMA(1,1)-GARCH(1,1) time series models to each
## margin and extract the standardized residuals which can then be
## investigated for cross-sectional dependence (de-GARCHing). Since
## the marginal fitting of so many time series models can fail, we use
## the fail-safe implementation fit_ARMA_GARCH() in qrmtools. The
## appearing warnings for six time series can be ignored (they come
## from finding initial values).
file <- paste0("SP500_",paste0(time, collapse = "--"),
               "_X_Z_nu_sectors_ssectors.rda")
if(file.exists(file)) {
    load(file)
} else {
    X <- -returns(x) # -log-returns
    uspec <- rep(list(ugarchspec(distribution.model = "std")),
                 ncol(X))
    system.time(fit.ARMA.GARCH <-
                    fit_ARMA_GARCH(X, ugarchspec.list = uspec)) # ~ 2.5min
    ...
    resi <- lapply(fits, residuals, standardize = TRUE) # grab out
                                                        # residuals
    Z <- as.matrix(do.call(merge, resi)) # standardized residuals
    stopifnot(is.matrix(Z), nrow(Z) == 755, ncol(Z) == 461) # fail-safe
    colnames(Z) <- colnames(x)
    nu.margins <- vapply(fits,
                         function(x) x@fit$coef[["shape"]],
                         NA_real_) # vector of estimated df
    save(X, Z, nu.margins, sectors, ssectors,
         file = file, compress = "xz") # save computed objects
}
n <- nrow(X) # 755
d <- ncol(X) # 461
stopifnot(n == 755, d == 461) # fail-safe programming


### 3 Pseudo-observations ############################################

## Order the data according to business sectors *and* subsectors
ord <- order(sectors, ssectors) # order according to sectors, subsectors
...
U <- pobs(Z[,ord])
## the part used in the recursive binning demonstration
saveRDS(U, "sp500pseudo.Rds")
...
if(file.exists(file)) {
    ...
    ## Pairwise nonparametric estimating of the upper tail-dependence
    ## coefficient
    system.time(LamPWnp <- fitLambda(U, p = 0.1, lower.tail = FALSE,
                                     verbose = TRUE)) # ~ 2min
    diag(LamPWnp) <- NA
    ## Save object
